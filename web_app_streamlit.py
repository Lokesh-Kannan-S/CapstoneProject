...
def main():
    ...
    data_selection = st.radio("Choose a dataset:",["category Data", "Complexes Data", "competitor Data"], horizontal=True) # Data Selection in single row

    # --- Analysis based on Data Selection ---
    if data_selection == "category Data":
        selected_df = df_category
        st.header("category Data Analysis")
        st.subheader("category Data Filters") # Added Subheader
    

        # --- Summary Statistics for Competitor Data ---
        st.subheader("category Data Summary")
        col1, col2 = st.columns(2)
        total_competitions = selected_df["competition_id"].nunique()
        ...
        st.subheader("Filtered category Data")
    ...
    elif data_selection == "competitor Data":
        selected_df = df_competitor
        st.header("competitor Data Analysis")
        st.subheader("competitor Data Filters")
    

        # --- Summary Statistics ---
        st.subheader("competitor Data Summary")
        col1, col2, col3 = st.columns(3)
        total_competitors = selected_df["name"].nunique()
        ...
        min_rank, max_rank = int(selected_df["rank"].min()), int(selected_df["rank"].max())
        ...
        filtered_df = selected_df[
            selected_df["name"].str.contains(competitor_search_term, case=False, na=False)
            & (selected_df["rank"] >= rank_range[0])
            & (selected_df["rank"] <= rank_range[1])
        ...
        competitor_names = ["Select Competitor"] + sorted(selected_df["name"].unique().tolist())
        selected_competitor = st.selectbox("Select a Competitor", competitor_names)

        if selected_competitor != "Select Competitor":
            competitor_details = selected_df[selected_df["name"] == selected_competitor]
        ...
        top_ranked = selected_df.sort_values("rank").head(10)
        ...
        col_leaderboard1.dataframe(top_ranked[["S.No","name", "rank"]])
        ...
        col_leaderboard2.dataframe(highest_points_df[["S.No","name", "points"]])
